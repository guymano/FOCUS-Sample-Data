...
Run: python -m unittest discover -s generators -p 'test_focus_1_3_regressions.py'
"""
import copy
...
class RegressionTests(unittest.TestCase):
    ...
    def test_validator_coexists_with_generic_sample_files(self):
        ...
        import validate_focus_1_3_samples as validator
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(validator, "DATA", data), contextlib.redirect_stdout(io.StringIO()):
                self.assertEqual(6, len(validator.sample_paths()))
    ...
    def test_exact_decimal_json(self):
        for module in self.modules.values():
            value = Decimal("274.93248601305144")
            quantity = Decimal("9007199254740993.000000001")
            raw = module._contract_applied('c"\\\n', "contract", value, quantity, "Hours")
            element = json.loads(raw, parse_float=Decimal, parse_int=Decimal)["Elements"][0]
            self.assertEqual(element["ContractCommitmentAppliedCost"], value)
            self.assertEqual(element["ContractCommitmentAppliedQuantity"], quantity)
            self.assertEqual(element["ContractCommitmentId"], 'c"\\\n')
            self.assertEqual(module._s_decimal(Decimal(8760)), "8760.0")
            self.assertEqual(module._s_decimal(Decimal("12.345")), "12.345")
            self.assertEqual(len(json.loads(module._contract_applied("id", "contract", value))["Elements"][0]), 3)
            for invalid in (Decimal("NaN"), Decimal("Infinity")):
                with self.assertRaises(ValueError):
                    module._decimal_json(invalid)
            with self.assertRaises(TypeError):
                module._decimal_json(0.1)

    def test_contract_exports_and_corruptions(self):
        for provider, module in self.modules.items():
            for size in (1, 23, 24, 1000, 1001):
                for seed in (0, 42, module.DEFAULT_SEED):
                    with self.subTest(provider=provider, size=size, seed=seed):
                        rows = module.generate_rows(size, seed)
                        contracts = module.generate_contract_commitment_rows(size, seed)
                        self.assertEqual(audit.audit_rows(rows, provider, contracts), [])
                        self.assertEqual(module.generate_contract_commitment_csv_bytes(size, seed),
                                         module.generate_contract_commitment_csv_bytes(size, seed))
            rows = copy.deepcopy(self.samples[provider])
            row = next(r for r in rows if r["ContractApplied"])
            data = json.loads(row["ContractApplied"])
            data["Elements"][0]["ContractCommitmentAppliedCost"] = 999
            row["ContractApplied"] = json.dumps(data)
            self.assertTrue(any(e.startswith("contract:") for e in audit.audit_rows(rows, provider)))
            contracts = module.generate_contract_commitment_rows(1000, module.DEFAULT_SEED)
            quantity_row = next(r for r in contracts if r["ContractCommitmentQuantity"])
            quantity_row["ContractCommitmentQuantity"] = "8760"
            self.assertTrue(any(e.startswith("contract:") for e in audit.audit_rows(self.samples[provider], provider, contracts)))
            contracts = module.generate_contract_commitment_rows(1000, module.DEFAULT_SEED)
            for contract in contracts:
                contract["ContractId"] = "wrong-contract"
            self.assertTrue(any(e.startswith("contract:") for e in audit.audit_rows(self.samples[provider], provider, contracts)))
    ...
    def test_contract_semantics_reject_inapplicable_terms_and_shapes(self):
        for provider, module in self.modules.items():
            original = self.samples[provider]
            contracts = module.generate_contract_commitment_rows(1000, module.DEFAULT_SEED)
            for mutation in ("extra", "missing_quantity", "wrong_service", "duplicate_key", "wrong_unit"):
                rows = copy.deepcopy(original)
                row = next(r for r in rows if r["ContractApplied"] and "COMPUTE-0003" in r["ContractApplied"])
                obj = audit.exact_json(row["ContractApplied"])
                element = obj["Elements"][0]
                if mutation == "extra":
                    element["Unexpected"] = True
                elif mutation == "missing_quantity":
                    del element["ContractCommitmentAppliedQuantity"]
                    del element["ContractCommitmentAppliedUnit"]
                elif mutation == "wrong_service":
                    row["ServiceName"] = audit.STORAGE_SERVICE[provider]
                elif mutation == "wrong_unit":
                    element["ContractCommitmentAppliedUnit"] = "GB"
                row["ContractApplied"] = module._decimal_json(obj)
                if mutation == "duplicate_key":
                    row["ContractApplied"] = row["ContractApplied"].replace('{"Elements":', '{"Elements":[],"Elements":')
                self.assertTrue(any(e.startswith("contract:") for e in audit.audit_rows(rows, provider, contracts)), mutation)
            for mutation in ("period", "annual_cost", "annual_quantity"):
                broken = copy.deepcopy(contracts)
                ids = {r["CommitmentDiscountId"] for r in original if r["CommitmentDiscountCategory"] == "Usage"}
                term = next(r for r in broken if r["ContractCommitmentId"] in ids)
                if mutation == "period":
                    term["ContractCommitmentPeriodEnd"] = term["ContractCommitmentPeriodStart"]
                elif mutation == "annual_cost":
                    term["ContractCommitmentCost"] = str(Decimal(term["ContractCommitmentCost"]) / 500)
                else:
                    term["ContractCommitmentQuantity"] = "8760.0"
                self.assertTrue(any(e.startswith("contract:") for e in audit.audit_rows(original, provider, broken)), mutation)
