...
def snapshot_errors(source, expected):
    ...
    generator = ROOT / "generators" / f"generate_{provider}_focus_1_3.py"
    errors = []
    ...
    for key, value in source_manifest(ROOT, "1_3", provider).items():
...
def sample_paths():
    ...
    names += [f"focus_sample_contractcommitment_{p}.csv" for p in audit.PROVIDERS]
...
def main(argv=None):
    ...
    if not args.check_existing:
        ...
        for name, key in (("model-1.3.0.1.json", "model_sha256"), ("currency_codes.csv", "currency_codes_sha256")):
    ...
    for source in sample_paths():
        ...
        if args.check_existing:
            ...
            command = [sys.executable, "-m", "focus_validator.main", "--data-file", str(source),
                       "--validate-version", "1.3.0.1", "--focus-dataset", dataset,
        ...
        if args.record:
            ...
            candidate.update(data_sha256=digest(source), generator_sha256=digest(ROOT / "generators" / f"generate_{provider}_focus_1_3.py"),
                             model_sha256=runtime["model_sha256"], currency_codes_sha256=runtime["currency_codes_sha256"])
            candidate.update(source_manifest(ROOT, "1_3", provider))
            recorded[source.name] = candidate
            revision = subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=ROOT, text=True).strip()
            manifest = dict(candidate, dataset=dataset, model="1.3.0.1", validator=runtime["validator"],
                            base_revision=revision, result=report, exit_code=0,
                            command="focus-validator --data-file FOCUS-1.3/" + source.name + " --validate-version 1.3.0.1 --focus-dataset " + dataset + " --applicability-criteria ALL --show-violations --block-download")
