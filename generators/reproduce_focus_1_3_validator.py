...
def main(argv=None):
    ...
    if args.run:
        ...
        model_path = package / "rules/model-1.3.0.1.json"
    ...
    for name, (number, row), targets in cases:
        ...
        if args.run:
            command = [sys.executable, "-m", "focus_validator.main", "--data-file", str(file),
                       "--validate-version", "1.3.0.1", "--focus-dataset", "CostAndUsage",
            ...
            meta.update(observed_target_failures={key: report["failures"][key] for key in targets},
                        ...
                        command="focus-validator --data-file " + name + ".csv --validate-version 1.3.0.1 --focus-dataset CostAndUsage --applicability-criteria ALL --show-violations --block-download")
